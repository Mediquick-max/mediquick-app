...
export async function generateImageBuffer(
  ...
  if (!openai) throw new Error("AI integration not configured");
  const response = await openai.images.generate({ model: "gpt-image-1", prompt, size } as any);
...
export async function editImages(imageFiles: string[], prompt: string, outputPath?: string): Promise<Buffer> {
  ...
  const response = await openai.images.edit({ model: "gpt-image-1", image: images, prompt } as any);
