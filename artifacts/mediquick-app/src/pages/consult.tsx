...
import {
  ...
  Award, Languages, MapPin, Heart, RefreshCw, CreditCard, Banknote, Tag
} from "lucide-react";

const DOCTOR_PLAN_DISCOUNTS: Record<string, number> = {
  free: 0, gold: 0, platinum: 0, yearly: 0.10,
};
const DOCTOR_PLAN_LABELS: Record<string, string> = {
  gold: "", platinum: "", yearly: "10% off (Yearly Plan)",
};
...
export default function ConsultPage() {
  ...
  const [form, setForm] = useState({
    ...
    healthIssue: "", consultationType: "video", paymentMethod: "online" as "online" | "cash"
  });
          ...
          <div className="bg-card rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl border border-border">
            ...
            <form onSubmit={handleBook} className="p-5 space-y-4">
              ...
              {(() => {
                ...
                const rate = form.paymentMethod === "online" ? (DOCTOR_PLAN_DISCOUNTS[plan] ?? 0) : 0;
                const discount = Math.round(selectedDoctor.fee * rate);
                const finalFee = selectedDoctor.fee - discount;
                return (
                  <div className={`rounded-2xl p-3 border ${form.paymentMethod === "cash" ? "bg-amber-50 border-amber-200" : "bg-primary/5 border-primary/20"}`}>
                    {form.paymentMethod === "cash" ? (
                      <>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-semibold">Consultation Fee (Cash)</span>
                          <span className="font-bold text-foreground text-lg">₹{selectedDoctor.fee}</span>
                        </div>
                        <p className="text-xs text-amber-700 mt-1.5 flex items-start gap-1">
                          <Banknote className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                          Doctor ko seedha cash dein. Subscription discount cash par apply nahi hoga.
                        </p>
                      </>
                    ) : discount > 0 ? (
                      <>
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground line-through">₹{selectedDoctor.fee}</div>
                          </div>
                          <div className="text-right">
                            <div className="font-bold text-primary text-lg">₹{finalFee}</div>
                            <div className="text-xs text-emerald-600 font-bold flex items-center gap-1">
                              <Tag className="w-3 h-3" /> Save ₹{discount} ({Math.round(rate * 100)}% plan)
                            </div>
                          </div>
                        </div>
                      </>
              ...
              <button type="submit" disabled={bookingLoading || !form.date || !form.timeSlot || !form.healthIssue}
                className={`w-full py-3.5 rounded-2xl font-bold active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${
                  form.paymentMethod === "cash"
                    ? "bg-emerald-600 hover:bg-emerald-700 text-white"
                    : "bg-primary hover:bg-primary/90 text-primary-foreground"
                }`}>
                {bookingLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Processing...</> :
                  form.paymentMethod === "cash"
                    ? <><Banknote className="w-5 h-5" /> Confirm (Cash Payment)</>
                    : <><CreditCard className="w-5 h-5" /> Confirm & Pay ₹{selectedDoctor.fee - Math.round(selectedDoctor.fee * (DOCTOR_PLAN_DISCOUNTS[user?.plan ?? "free"] ?? 0))}</>
                }
              </button>
            </form>
          </div>
          ...
          <div className="bg-card rounded-3xl p-8 max-w-sm w-full shadow-2xl border border-border text-center space-y-4">
            ...
            <div className="bg-secondary/40 rounded-2xl p-4 text-left space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-primary">₹{bookingSuccess.amountPaid}</span>
                  {bookingSuccess.paymentMethod === "cash" ? (
                    <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-lg font-semibold">Cash</span>
                  ) : bookingSuccess.discountAmount > 0 ? (
                    <span className="text-xs bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-lg font-semibold">Plan Discount ✓</span>
                  ) : (
                    <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-lg font-semibold">Online</span>
                  )}
                </div>
              </div>
              {bookingSuccess.paymentMethod === "cash" && (
                <div className="text-xs text-amber-700 bg-amber-50 rounded-xl p-2 flex items-start gap-1.5">
                  <Banknote className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                  Doctor ko seedha cash dein. Platform fee doctor ke account se deduct hogi.
                </div>
              )}
            </div>
