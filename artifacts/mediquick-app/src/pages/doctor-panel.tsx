...
export default function DoctorPanelPage() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "profile" | "appointments" | "payments" | "plans" | "medicines">("dashboard");
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-3xl border border-border/50 shadow-sm p-5 space-y-4">
              ...
              <div className="bg-primary/5 border border-primary/20 rounded-2xl p-3 text-xs text-muted-foreground">
                Apne clinic ka medicine shop setup karein. Patients 5km ke andar apki listed medicines order kar sakte hain.
              </div>
              {shopLoading ? (
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-semibold text-muted-foreground mb-1">Shop / Clinic Name *</label>
                      <input value={shopForm.shopName} onChange={e => setShopForm(f => ({ ...f, shopName: e.target.value }))}
                        placeholder="Dr. Sharma Clinic Pharmacy"
                        className="w-full px-3 py-2.5 rounded-2xl border border-border bg-secondary/30 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input value={shopForm.pincode} onChange={e => setShopForm(f => ({ ...f, pincode: e.target.value }))}
                  ...
                  <button onClick={detectShopLocation} disabled={geoLoading}
                    ...
                    {geoLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Detecting location...</> : <><Navigation className="w-4 h-4" /> Detect Shop Location (for delivery range)</>}
                  </button>
                  {shopLat && shopLng && (
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">Shop Name</span><span className="font-semibold">{shopProfile.shopName}</span></div>
                  ...
                  <div className="flex justify-between"><span className="text-muted-foreground">City</span><span className="font-semibold">{shopProfile.city || "—"}</span></div>
                  <div className="flex items-center gap-1.5 text-xs mt-1">
                    {shopProfile.lat ? <><CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /><span className="text-emerald-700">Location set — visible to patients within 5km</span></> : <><AlertCircle className="w-3.5 h-3.5 text-amber-500" /><span className="text-amber-700">Location not set — click Edit to add location</span></>}
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="bg-white rounded-3xl border border-border/50 shadow-sm p-5 space-y-4">
                ...
                {shopMeds.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-muted-foreground">No medicines listed yet. Click "Add Medicine" to start.</p>
                  </div>
                ) : (
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 text-sm text-blue-800">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs">Patients within 5km will see your listed medicines and can place orders. You deliver the medicine yourself and collect cash. ₹10 per 100m delivery charge (₹1 platform fee included).</p>
                </div>
              </div>
            </div>
          </div>
